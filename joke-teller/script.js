...
function test(joke, joke1){
    VoiceRSS.speech({
        key: 'd30f98b59dec43c3b1b4942d6cf700f5',
        src: `${joke}  ${joke1}`,
        hl: 'en-us',
        v: 'Linda',
        r: 0, 
...
async function getJokes(){
    try{
        ...
        console.log(data);
        test(data.setup, data.delivery);
        showJoke(data.setup, data.delivery)
    }
    catch(error){
        // error here
...
function showJoke(joke, joke1){
    jokeBox.innerHTML = `<h1>Qsn. ${joke}</h1> <h2>Qsn. ${joke1}</h2>`
}
